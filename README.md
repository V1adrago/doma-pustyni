...
> Версия: v2.7 — Уровень 2, рулетка домов, Пустынные Кланы
...
### v2.7 — Уровень 2, рулетка домов, Пустынные Кланы

- уровень 2 открывается с рейтинга 100; локация меняется на «Соляные разломы» (`salt_rifts`)
- при достижении уровня 2 Пустынные Кланы (`desert_clans`) добавляются в рулетку
- игрок получает 1 бесплатный прокрут; дом не выдаётся сразу — только из рулетки
- навыки дома бесплатны после получения; дом можно подключить в меню
- выбранный дом применяется в бою через `FactionManager`
- `profile-service.js`: миграция старых профилей без сброса рейтинга/побед
- `menuState.js`: 4 состояния фракций (locked / roulette_available / owned / active)
- `factions.js`: добавлен `desert_clans` с цветами башен и пассивкой «След песка»
...
**Проблема:** шаги обучения показывались по таймеру независимо от игрока — карты исчезали пока он читал, подсказки про «Ищейку» появлялись когда той уже не было в руке.

**Решение — sequential шаги с блокировкой:**

Шаги перешли с time-based на очередь: каждый следующий шаг показывается только после завершения предыдущего.

| Режим шага | Кнопка | Поведение |
|------------|--------|-----------|
| Информационный | «Понятно →» | Игра на паузе, после нажатия — следующий шаг (пауза не снимается между шагами) |
| С действием (`requireCardPlay: true`) | «Сделать это →» | Пауза снята, карты подсвечены, ждём пока игрок реально сыграет карту → пауза и следующий шаг |

**Страховка специй:** при переходе в режим ожидания карты (`requireCardPlay`) контроллер проверяет специи игрока. Если не хватает на минимальную из подсвеченных карт — автоматически добавляет нужное. Застрять без ресурсов невозможно.

**Имена карт на карточках:** убран `white-space: nowrap`, добавлен перенос на 2 строки (`white-space: normal; word-break: break-word; font-size: 8.5px`). Полные каноничные имена («Ищейка Барханов», «Клинок Дома» и т.д.) теперь помещаются в карточку шириной 68px.

**Тексты подсказок** используют короткую форму + иконку: «Поставь Клинка (⚔) или Ищейку (⚡)» — визуальная связка с иконкой на карточке.

| Файл | Что изменилось |
|------|----------------|
| `src/tutorial/tutorial-controller.js` | Sequential шаги, `_awaitingStep`/`_awaitingAction`, `_topUpIfNeeded()`, колбэк `getPlayerEconomy` |
| `src/tutorial/tutorial-overlay.js` | `TutorialBattleHint.show()` — добавлен `actionMode` (кнопка «Сделать это →») и `autoDismissMs` |
| `src/tutorial/tutorial-data.js` | Шаги уроков 1 и 2 переписаны: убран `time`, добавлен `requireCardPlay: true` на ключевых шагах; `completionCondition.seconds` увеличен до 90/100 |
| `src/main.js` | `getPlayerEconomy: () => playerEconomy` добавлен в callbacks туториала |
| `style.css` | `.ch-name` — `white-space: normal`, `word-break: break-word`, `font-size: 8.5px`, `line-height: 1.25` |
